...
def random_gif(search_term, lmt=8):
    r = requests.get(
        f"https://api.tenor.com/v1/random?q={search_term}&key={TENSOR_API_KEY}&limit={lmt}"
    )
    ...
    r = requests.get(
        f"https://api.tenor.com/v1/registershare?id={gif_id}&key={TENSOR_API_KEY}&q={search_term}"
    )
